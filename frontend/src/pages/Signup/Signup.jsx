...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    if (password !== confirmPassword) {
      return showNotification("Password's should match", "error");
    }
    if (emptyDetails) {
      return showNotification("Fill all details", "error");
    }

    e.preventDefault();
    try {
      setLoading(true);
      const result = await postFetch("users/register", {
        fullName,
        email,
        password,
      });
  ...
  return (
    <section className="h-dvh w-dvw flex items-center justify-center bg-gray-100">
      <div className="px-5 w-[90%] md:w-2/3 lg:w-1/2 gap-2 flex flex-col shadow rounded-lg bg-gray-200 py-10 sm:py-20">
        ...
        <div className="mb-5 relative items-center">
